...
//             ignoreImage: true
//         });
//         const cleanNoteContent = htmlToText(noteContent || '', {
//             wordwrap: false,
//             ignoreHref: true,
//             ignoreImage: true
//         });
//         const cleanKfAnalysisContent = htmlToText(kfAnalysisContent || '', {
//             wordwrap: false,
//             ignoreHref: true,
//             ignoreImage: true
//         });
//         const cleanOutlineContent = htmlToText(outlineContent || '', {
//             wordwrap: false,
//             ignoreHref: true,
//             ignoreImage: true
//         });
//         const chatHistoryContent = chatHistory ? JSON.stringify(chatHistory) : '';
...
//             text: { content: chunk }
//         }));
//         const noteChunks = splitTextIntoChunks(cleanNoteContent, 2000);
//         const noteRichText = noteChunks.map(chunk => ({
//             text: { content: chunk }
//         }));
//         const kfAnalysisChunks = splitTextIntoChunks(cleanKfAnalysisContent, 2000);
//         const kfAnalysisRichText = kfAnalysisChunks.map(chunk => ({
//             text: { content: chunk }
//         }));
//         const outlineChunks = splitTextIntoChunks(cleanOutlineContent, 2000);
//         const outlineRichText = outlineChunks.map(chunk => ({
//             text: { content: chunk }
//         }));
//         const chatHistoryChunks = splitTextIntoChunks(chatHistoryContent, 2000);
...
//                 '議論文內容': { rich_text: essayRichText },
//                 '筆記內容': { rich_text: noteRichText },
//                 'KF摘要': { rich_text: kfAnalysisRichText },
//                 '聊天歷史紀錄': { rich_text: chatHistoryRichText },
//                 '寫作大綱': { rich_text: outlineRichText },
//             },
//         });
...
//             filter: {
//                 and: [
//                     { property: '學生姓名', title: { equals: studentName } },
//                     { property: '班級', rich_text: { equals: className } },
//                     { property: '主題', rich_text: { equals: theme } },
//                 ],
//             },
//         });
...
//         const essayContent = page.properties['議論文內容']?.rich_text?.map(t => t.text.content).join('') || '';
//         const noteContent = page.properties['筆記內容']?.rich_text?.map(t => t.text.content).join('') || '';
//         const kfAnalysisContent = page.properties['KF摘要']?.rich_text?.map(t => t.text.content).join('') || '';
//         const chatHistoryContent = page.properties['聊天歷史紀錄']?.rich_text?.map(t => t.text.content).join('') || '';
//         const outlineContent = page.properties['寫作大綱']?.rich_text?.map(t => t.text.content).join('') || '';

//         // 將聊天歷史紀錄從 JSON 字串轉回物件
//         let chatHistory = [];
//         try {
//             chatHistory = chatHistoryContent ? JSON.parse(chatHistoryContent) : [];
//         } catch (error) {
//             console.error('解析聊天歷史紀錄失敗:', error.message);
//         }
